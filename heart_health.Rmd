...
data_split <- initial_split(heart, prop = 0.75)
...
cv_tune <- cv_data %>%  
  crossing(mtry = c(2, 4, 8, 13)) # mtry range 1:# of features

cv_models_rf <- cv_tune %>% 
  mutate(model = map2(train, mtry, ~ranger(formula = Target~., 
                                           data = .x, mtry = .y, 
                                           num.trees = 100, seed = 1337)))
...
best_model <- ranger(formula = Target~., data = training_data , mtry = 4, num.trees = 100, seed = 1337)
...
# predict(best_model, testing_data, type = "response") > 0.5
# but seems model is 100% accurate, this doesn't seem right.
# Could try some more testing, or more splits of data for testing.
